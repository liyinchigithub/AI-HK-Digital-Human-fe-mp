<template>
	<view>
		...
		<u-overlay :show="overlayShow">
			<view class="warp">
				<u-loading-icon inactive-color="#409EFF" size="70" mode="semicircle" textSize="40" text="加载中"></u-loading-icon>
			</view>
		</u-overlay>
		<!-- 滚动通知 -->
		<view class="" v-show="noticeBarShow">
			<u-notice-bar mode="closable" icon="volume" bgColor="#fa3534" color="#ffffff" :text="noticeText" speed="100"
				fontSize="30"></u-notice-bar>
		</view>
		<!-- 轮播图-->
		<view class="swiper">
			<u-swiper :list="bannerList" @click="bannerClick()" keyName="image" showTitle :autoplay="true" circular
				height="450" duration="1000" indicatorMode="line"></u-swiper>
		</view>
		<!-- 搜索 -->
		<view class="">
		...
		<!-- 需求发布 -->
		<view class="releaseRequire">
			<uni-card :is-shadow="true" is-full @click="goto('/pages/business/releaseInfo/releaseInfo')"  >
				<u-row customStyle="margin-bottom: 0px" justify="space-between" gutter="10">
					<u-col>
						<text style="margin-left: 40%;font-size:40rpx;color: black;">发布需求</text>
					</u-col>
				</u-row>
				<u-row customStyle="margin-bottom: 10px" justify="space-between" gutter="10">
					<u-col>
						<text style="margin-left: 30%;font-size:30rpx;color: black;">足不出户，发布需求</text>
					</u-col>
				</u-row>
				<image style="border-radius:2%" :src="releaseImageSrc" mode="widthFix"></image>
			</uni-card>
		</view>
		<!-- 拼图（分类） todo -->
		...
		<view class="settleIn;" @click="goto('/pages/business/settleIn/settleIn')">
			...
			<image class="image" :src="settleInImageSrc" mode="heightFix"></image>
		</view>
		...
		<view class="" v-for="(item,index) in listData" :key="index">
			<uni-card :title="item.classTypeName" :extra="item.createDate"
				...
				<u-row justify="space-between" gutter="15">
					<u-col span="3">
						<u--image :showLoading="true" :src="item.imageList[0].url" width="80px" height="80px"
							@click="previewImageSingle(item.imageList.url)"></u--image>
					</u-col>
					<u-col span="5">
					</u-col>
					<u-col span="3">
...
export default {
  ...
  methods: {
    ...
    bannerClick(index) {
      ...
      if (this.bannerList[index].isToUrl) {
        ...
        // 判断是否包含https，包含则直接跳转https地址，不包含则跳转路由页面地址
        if (this.bannerList[index].toUrl.indexOf("https") != -1) {
          //  跳转自定义URL（需要公众号加入域名白名单）
          uni.navigateTo({
            url: `/pages/business/webView/webView?webUrl=${this.bannerList[index].toUrl}`,
          });
        } else {
          uni.navigateTo({
    ...
    getSetInImage() {
      ...
      uniCloud.callFunction({
        ...
        data: {
          method: "getHomeSetInImage",
          isEnable: true,
        },
        success: (res) => {
          console.log("getBasicInformation res.result.data:", res.result.data);
          // 金刚区数据
          that.settleInImageSrc = res.result.data[0].image.url;
        },
      });
    },
    // 发布需求模块图片
    getReleaseImage() {
      ...
      uniCloud.callFunction({
        name: "getBasicInformation",
        data: {
          method: "getHomeReleaseImage",
          isEnable: true,
        },
        success: (res) => {
          console.log(
            "getBasicInformation getHomeReleaseImage res.result.data:",
            res.result.data
          );
          // 金刚区数据
          that.releaseImageSrc = res.result.data[0].image.url;
          // 关闭遮罩
    ...
    getBasicInforMation() {
      ...
      uniCloud.callFunction({
        ...
        success: (res) => {
          console.log("res.result.data[0]:", res.result.data);
        },
      });
    },
    // 首页顶部公告消息
    ...
    getDeivceWidthHeight() {
      this.deivceHeight = uni.getSystemInfoSync().windowHeight;
      this.deivceWidth = uni.getSystemInfoSync().windowWidth;
      console.log("deivceWidth", this.deivceWidth);
      console.log("deivceHeight", this.deivceHeight);
    },
  },
  //添加控件依赖
  components: {
  ...
  onLoad() {
    ...
    this.getNoticeBar();
    // 设备屏幕宽高
    this.getDeivceWidthHeight();
    //  商家、个人入驻
    this.getSetInImage();
    //  发布需求模块图片
    this.getReleaseImage();
  },
  // 分享微信好友
  onShareAppMessage() {
    // TODO	读取缓存分享人ID
