	...
	<view>
			...
			<!-- 	<view style="margin-top:100rpx;">
				<u-row gutter="11">
					...
					<u-col span="4" offset="3">
						<uni-icons type="chat-filled" color="#FFFFFF" size="30" @click="goto('/pages/notice/notice')">
						</uni-icons>
					</u-col>
				</u-row>
			</view> -->
			...
			<view class="box">
				<view class="label" v-for="(row,index) in orderTypeLise" :key="row.name" hover-class="hover"
					...
					<view class="icon">
						...
						<view class="">
							<image
								:src="'https://mp-e51575a6-5952-4ad5-bb31-45ff909d7f65.cdn.bspapp.com/cloudstorage/'+row.icon">
							</image>
						</view>
					</view>
					<!-- 文字（全部、待支付、已完成、已取消） -->
		...
		<view class="list" v-for="(list,list_i) in severList" :key="list_i">
			<view class="li" v-for="(li,li_i) in list" @tap="toPage(list_i,li_i)"
				...
				<view class="icon">
					<image
						:src="'https://mp-e51575a6-5952-4ad5-bb31-45ff909d7f65.cdn.bspapp.com/cloudstorage/'+li.icon">
					</image>
				</view>
		...
		<button open-type="contact" style="background-color: #FFFFFF;">
			<image
				...
				mode="widthFix" style="height:46rpx;width: 42rpx ;margin-top:30rpx;margin-right:20rpx;">
				<text>联系客服</text>
			</image>
		</button>
	</view>
	...
	export default {
		...
		methods: {
			...
			toOrderType(index) {
				...
				// 路由跳转
				this.goto(`/pages/order/order?orderStatus=${index}`);
			},
			//用户点击列表项
			toPage(list_i, li_i) {
				// uni.showToast({
				...
				switch (list_i) {
					...
					case 0:
						switch (li_i) {
							case 0:
								// 页面跳转
								this.goto("/pages/coupon/coupon")
								break;
							case 1:
								this.goto("/pages/notice/notice")
								break;
							case 2:
								this.goto("/pages/myCollect/myCollect")
								break;
							case 3:
								this.goto("/pages/test/test")
								break;
							case 4:
								this.goto("/pages/myCard/myCard")
								break;
							default:
								uni.showToast({
									title: "没有此项目"
			...
			scan() {
				...
				uni.scanCode({
					...
					success: (res) => {
						...
						uni.navigateTo({
							url: `/pages/scanCode/scanCode?scanResult=${res.result}`
						});
