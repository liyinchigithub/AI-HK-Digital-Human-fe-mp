		...
		<u--form labelPosition="top" :model="formData" :rules="rules" ref="form">
			...
			<uni-card :is-shadow="true" is-full>
				...
				<u-form-item labelWidth="150" prop="userInfo.gender" borderBottom ref="item1">
					<u-radio-group :labelSize="40" :iconSize="40" shape="circle" v-model="formData.userInfo.gender" placement="column" @change="genderGroupChange">
						<u-radio :labelSize="40" :iconSize="40"  :customStyle="{marginBottom: '8px'}" v-for="(item, index) in genderRadiolist"
							:key="index" :label="item.name" :name="item.name" @change="genderRadioChange">
						</u-radio>
					</u-radio-group>
				</u-form-item>
			</uni-card>
			...
			<uni-card :is-shadow="true" is-full>
				...
				<u-form-item labelWidth="150" prop="userInfo.contactAddress" borderBottom ref="item1">
					<u--input type="string" width="50" clearable v-model="formData.userInfo.contactAddress"
						border="surround" @clear="clearInputTitle"></u--input>
				</u-form-item>
			</uni-card>
			<!-- 身份证号 -->
			<uni-card :is-shadow="true" is-full>
			...
			submit(form) {
				this.$refs[form].validate().then(res => {
					...
					uniCloud.callFunction({
						...
						success: (res) => {
							console.log("addIndividualEntry success",res.result.data);
							uni.showToast({
								title: `提交成功`
							})
						},
