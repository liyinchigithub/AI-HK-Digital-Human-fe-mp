	...
	<view>
		...
		<cp-goods-select height="100vh" :props="{label:'label',value:'value',children:'children'}" :options="list"
			...
			<cp-goods-item v-for="(item,index) in list" :key="index" :category="item.label">
				<view class="" v-for="(cell,key) in item.children" :key="key" class="goods__item ">
					<view class="cp-goods" @click="goto(`/pages/business/goodsDetail/goodsDetail?goodsID=${cell.goodsID}`)">
						<u-row>
							<!-- 商品图片 -->
							<u-col span="4">
