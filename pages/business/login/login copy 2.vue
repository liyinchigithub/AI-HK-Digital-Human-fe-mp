		...
		<view class="wrap">
			...
			<view class="content">
				...
				<view class="wexin_login_button">
					<u-button open-type="getPhoneNumber" @getphonenumber="getPhoneNumber" color="#71d5a1" shape="circle"
						text="微信登录"></u-button>
				</view>
			...
			uni.login({
				...
				success: async (wxcode) => {
					...
					var callFunctionResult = await uniCloud.callFunction({
						...
						data: {
							code: wxcode.code,
							encryptedData: that.encryptedData, // 从前端获取
							iv: that.iv // 从前端获取
						}
					});
					...
					that.phoneNumber = callFunctionResult.result.phoneNumber; // 获取手机号码
					console.log("openid", that.openid);
					console.log("session_key", that.session_key);
					console.log("phoneNumber", that.phoneNumber);
					uni.setStorageSync("openID", that.openid);
					uni.setStorageSync("session_key", that.session_key);
					uni.setStorageSync("phoneNumber", that.phoneNumber); // 缓存手机号码

					// 在获取到手机号码后调用 getUserInfo
					// that.getUserInfo();
					// 新增用户
					that.addUser();
					// 跳转我的页面
		...
		getBasicInforMation() {
			uniCloud.callFunction({
				...
				success: (res) => {
					...
					// console.log("home basicInformation:", res.result.data[0]);
				},
			});
		},
